...
import 'package:naegot/utils/get_location_from_latlng.dart';
...
class AddPlaceScreen extends StatefulWidget {
  ...
  final List<LatLng>? polygones;
  final double circleRadius;
  // final List<CircleMarker>? circleMarker;

  final MapPlaceType mapPlaceType;
              ...
              TextButton(
                onPressed: () async {
                  ...
                  if (formKey.currentState!.validate()) {
                    ...
                    try {
                      EasyLoading.show(status: "장소를 등록중입니다.");

                      loading = true;
                      setState(() {});
                      final doc = FirebaseSerivce.getDoc();

                      Place place = Place(
                        id: doc.id,
                        type: describeEnum(widget.mapPlaceType),
                        category: category ?? NO_CATEGORY_TEXT,
                      ...
                      switch (widget.mapPlaceType) {
                        case MapPlaceType.marker:
                          place.point = widget.marker;
                          break;
                        case MapPlaceType.line:
                          place.point = widget.polylines!.first;
                          place.polylines = widget.polylines;
                          break;
                        case MapPlaceType.circle:
                          place.point = widget.marker;
                          place.radius = widget.circleRadius.toInt();
                          break;
                        case MapPlaceType.polygon:
                          place.point = widget.polygones!.first;
                          place.polygons = widget.polygones;
                          break;
                      }
                      place.address = await getLocationFromLatlng(place);
                      final success = await FirebaseSerivce.addPlace(place);
                      if (success == false) {
                        return EasyLoading.showError("장소 등록을 실패하였습니다.");
                      }

                      EasyLoading.showSuccess("장소를 등록하였습니다.");
